 ...
 *              function will autoly to choose the property method to ajust to
 *              client Browswer. This js function mainly borrow from youku.com's
 *              uploader.min.js and try to re-create it for fullfiting my
 *              requirement.
 * @example 1. new Stream(); 2. var cfg = { extFilters : [".txt", ".gz"],
 *          fileFieldName : "FileData" }; new Stream(cfg);
 */ 
(function(){
	var Provider, nIdCount = 0, aOtherBrowsers = ["Maxthon", "SE 2.X", "QQBrowser"],
		...
		sTotalContainer = '<div id="#totalContainerId#" class="stream-total-tips">' +
			'	\u4e0a\u4f20\u603b\u8fdb\u5ea6\uff1a<span class="stream-process-bar" style="display:none;"><span style="width: 0%;"></span></span>' +
	 ...
	 * @forseReg boolean: when the
	 * @hostPrototype don't have the function, then register it. true: do it,
	 *                ignore the
	 * @hostPrototype whether has it. false: when not have it so that register
	 *                it.
	 */
	...
	SWFReference.isFlashVersionAtLeast = function(a, b, c) {return true;
		/*
		 * var d = parseInt(Browser.flashMajor, 10), e =
		 * parseInt(Browser.flashMinor, 10), f = parseInt( Browser.flashRev,
		 * 10), a = parseInt(a || 0, 10), b = parseInt(b || 0, 10), c =
		 * parseInt(c || 0, 10); return a === d ? b === e ? c <= f : b < e : a <
		 * d;
		 */
	};
					...
					for (var i = 0; i < entries.length; i++) {
						...
						if (entry.isFile) {
							...
							entry.file(function(f) {
								f.RelativePath = relativePath + f.name; /**
																		 * self
																		 * define
																		 * argument
																		 */
			...
			switch(event.type){
				case "load":
					...
					try {
						...
						/**
						 * the response can't process the request, so throws out
						 * the error.
						 */
	...
	Main.prototype = {
		...
		initializer : function() {
			...
			if (!this.config.customered) {
				...
				/** start1 */
				var filesQueuePanelTitlePercent = document.getElementById("upload_div_title_percent");
				/** end1 */
				...
				/** start2 */
				if(filesQueuePanelTitlePercent.hasChildNodes()){
					filesQueuePanelTitlePercent.innerHTML="";
				}
				filesQueuePanelTitlePercent.appendChild(totalQueue);
				/** end2 */
		...
		addStreamTask : function(a) {
			...
			/*
			 * bStreaming ? this.startPanel.style.display = "none" :
			 * (this.startPanel.style.height = "1px",
			 * this.startPanel.style.width = "1px");
			 */
		...
		cancelOne : function(file_id, stopping) {
			...
			if (!this.config.customered) {
				...
				/**
				 * * TODO: this code will be removed in the future. bStreaming ?
				 * this.startPanel.style.display = "block" :
				 * (this.startPanel.style.height = "auto",
				 * this.startPanel.style.width = "970px");
				 */
			}
		},
		...
		uploadFile : function(file, url, token, method) {
			if(0 != file.config.size){
				...
				alert(file.config.name+"大小为0字节，不能上传！");
				this.cancelOne(file.config.id);
			}
		},
