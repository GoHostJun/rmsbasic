$(function(){
	if(role==0){
	 ...
	 history.replaceState({}, "", window.location.href.split('?')[0] + "?_id="+shareId+"&userId="+userId+"&redictype="+type
			  +"&title="+title+"&imgUrl="+imgUrl+"&subbmit="+subbmit+"&loginid="+loginid
			  +"&nikename="+nikename+"&type="+type);
	//轮播
	  if("clues"==type){
		  getSBCues();
	  }else if("myclues"==type){
		  getMyCues()
	  }
	  $("#pass").click(function(){
		  addClub();
...
function getMyCues(){
	...
	$.ajax({
		type : 'POST',
		contentType : 'application/json',
		url : ctx +path+ "getCDVCueById/",
		data : onairMap.toJson(),
		cache : false,
		success : function(data) {
			if (0 == data.code) {
				 data=data.data;
				 title=toValue(data.assetname)
				 cueContent=toValue(data.content)
	...
	$.ajax({
		...
		success : function(data) {
			if (0 == data.code) {
				...
				$("#headtitle").text(toValue(data.assetname))
				$("#sourceDate").html(toValue(data.prostation)+"<span>"+toValue(data.creationdate)+"</span>")
				$("#cueContent").html("<p>"+toValue(data.content)+"</p>")
//				$("[name=imgThumb]").attr("src", data.thumbnailfileid); 
				if(data.thumbnailfileid&&data.thumbnailfileid.length>1){
					$.each(data.thumbnailfileid,function(i,item){
						thumbs.push(item.thumb);
						if(i==0){
