 ...
 * its file part, stored it. PS: use the `streaming api`, this will not store it
 * in a temporary file. {@link http
 * ://commons.apache.org/proper/commons-fileupload/streaming.html }
 */
public class FormDataServlet extends HttpServlet {
	...
	static final int MAX_FILE_SIZE = 1024 * 1024 * 1024 * 10;
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		/** flash @ windows bug */
		req.setCharacterEncoding(CODED_FORMAT);
		...
		try {
			...
			while (iter.hasNext()) {
				...
				if (item.isFormField()) {
					...
					value = new String(value.getBytes("ISO-8859-1"), CODED_FORMAT);
